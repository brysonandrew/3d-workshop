...
import { motion as motionDom, useMotionValue, MotionConfig, useSpring } from "framer-motion";
...
import { SphereGeometry, ShadowMaterial, SpotLight } from "three";
extend({ SphereGeometry, ShadowMaterial, SpotLight });
softShadows();

function Item({ isOn, setOn }: any): JSX.Element {
  ...
  return (
    <group scale={[1.25, 1.25, 1.25]} dispose={null}>
      ...
      >
        ...
        <motion.pointLight intensity={100} distance={1.4} variants={lightVariants} />
        <Shadow
          renderOrder={-1000}
          position={[0, -1, 0]}
...
export function useAnimatedText(target: number, textTransition: SpringOptions | undefined) {
  const ref = useRef(null);
  const value = useSpring(target, textTransition);

  useEffect(() => {
    if (ref.current) (ref.current as HTMLElement).innerText = target.toFixed(2);

    return value.onChange((v) => {
      if (ref.current) (ref.current as HTMLElement).innerText = v.toFixed(2);
    });
  });
...
export function Scene({ isOn, setOn }: any) {
  return (
    <MotionCanvas
      orthographic
      shadows
    ...
    >
      <motion.group animate={isOn ? "on" : "off"}>
  ...
  return (
    <Container>
      <MotionConfig transition={transition}>
        ...
        >
          <motionDom.h1 className="open" children="<h1>" />
          <motionDom.h1 className="close" children="</h1>" />
          <motionDom.h1 ref={headerRef} />
