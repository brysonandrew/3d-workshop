...
function Lighting({ isFullscreen }: any) {
...
export function Three({ isFullscreen }: any) {
  return (
    ...
    //     <meshPhysicalMaterial color="#e7b056" clearcoat={1} clearcoatRoughness={0} />
...
export const Orthogonal = () => {
  ...
  return (
    <Container data-is-fullscreen={isFullscreen} onClick={() => setIsFullscreen(!isFullscreen)}>
      <MotionConfig transition={{ duration: 4 }}>
        <Content>
          <motionDom.div className="canvas-container" layout>
            <Three isFullscreen={isFullscreen} />
          </motionDom.div>
          <motionDom.h1 layout animate={{ color: isFullscreen ? "#fff" : "#000" }}>
            {`Framer Motion`}
            <br />
            {`React Three Fiber`}
          </motionDom.h1>
          <motionDom.p layout animate={{ opacity: isFullscreen ? 0 : 1 }}>
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Suspendisse vel diam dapibus,
