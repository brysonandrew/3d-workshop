...
import { MeshPhongMaterial, BoxGeometry, Mesh, Group, PointLight } from "three";

extend({ MeshPhongMaterial, BoxGeometry, Mesh, Group, PointLight });

export function StarIcon() {
  return (
    <MotionCanvas shadows dpr={[1, 2]} camera={{ position: [0, 0, 5.5], fov: 45 }}>
      {lights.map(([x, y, z, intensity], index) => (
        <pointLight
          key={`light-${index}`}
          intensity={intensity}
